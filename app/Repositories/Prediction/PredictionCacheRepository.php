...
class PredictionCacheRepository extends BaseCacheRepository implements PredictionRepositoryInterface
{
    ...
    public function predictionComments($id, $params = []): string
    {
        $key = $this->generateKey([$id, $params]);

        return Cache::tags($this->tag)->remember($key, $this->timeToLive, function () use ($id, $params) {
          return  $this->repository->predictionComments($id, $params);
        });
    }
    ...
    public function allUserPredictions($userId = null, $params = [])
    {
        ...
        return Cache::tags($this->tag)->remember($key, $this->timeToLive, function () use ($userId, $params) {
            return  $this->repository->allUserPredictions($userId, $params);
        });
    }
    public function userCreatedPredictions($userId = null, $params = [])
    {
        $key = $this->generateKey([$userId = null, $params]);

        return Cache::tags($this->tag)->remember($key, $this->timeToLive, function () use ($userId, $params) {
            return  $this->repository->userCreatedPredictions($userId, $params);
        });
    }
